          ...
          <div className="card-body p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
              ...
              <select className="select select-bordered select-sm" value={newItem.unit} onChange={(e) => setNewItem({ ...newItem, unit: e.target.value })}>
                <option>دانه</option><option>متر</option><option>توپ</option><option>کیلو</option><option>لوله</option><option>ورق</option>
              </select>
              ...
              <tr key={item.id}>
                ...
                <td>
                  {editId === item.id && editField === 'quantity' ? (
                    ...
                    <span className={item.quantity <= 5 ? 'text-warning font-bold' : ''}>{formatNumber(item.quantity)}</span>
                  )}
                </td>
                <td>
                  {editId === item.id && editField === 'price_per_unit' ? (
                    ...
                    <span className={item.price_per_unit === 0 ? 'text-error font-bold' : ''}>{item.price_per_unit === 0 ? 'نامشخص' : formatCurrency(item.price_per_unit)}</span>
                  )}
                </td>
